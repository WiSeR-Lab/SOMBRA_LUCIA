...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_label(self, **kwargs):
        ...
        if 'anchors' in kwargs:
            ...
            anchors = self.generate_anchor_box()
        # (max_num)
        masks = kwargs['mask']
        ...
        assert gt_box_center_valid is not None
        # (n, 8, 3)
        gt_box_corner_valid = \
            box_utils.boxes_to_corners_3d(gt_box_center_valid,
                                          self.params['order'])
        #print(gt_box_corner_valid.shape)
        # (H*W*anchor_num, 8, 3)
        anchors_corner = \
    ...
    def post_process(self, data_dict, output_dict, output_stat=False):
        ...
        """
        # dict that stores number of proposals (bboxes)
        bbox_number_dict = {'original': 0,
                            'z_filter_out': 0,
                            's_filter_out': 0,
                            'before_nms': 0}
        
        exec_time_dict = {'z_filter': 0,
                          's_filter': 0,
                          'nms': 0}
        ...
        if len(pred_box2d_list) ==0 or len(pred_box3d_list) == 0:
            if not output_stat:
                ...
                return None, None, bbox_number_dict, exec_time_dict
        # shape: (N, 5)
        pred_box2d_list = torch.vstack(pred_box2d_list)
        bbox_number_dict['original'] += int(len(pred_box2d_list))
        #print(f'total bbx before postproc {len(pred_box2d_list)}')
        # scores
        ...
        torch.cuda.synchronize()
        exec_time_dict['s_filter'] = time.time() - start
        bbox_number_dict['s_filter_out'] += int(torch.sum(keep_index_1))

        #print(f'large removal: {torch.sum(keep_index_1)}')
        torch.cuda.synchronize()
        start = time.time()
        keep_index_2 = box_utils.remove_bbx_abnormal_z(pred_box3d_tensor)
        torch.cuda.synchronize()
        exec_time_dict['z_filter'] = time.time() - start
        bbox_number_dict['z_filter_out'] += int(torch.sum(keep_index_2))
        #print(f'z removal: {torch.sum(keep_index_2)}')

        keep_index = torch.logical_and(keep_index_1, keep_index_2)
        bbox_number_dict['before_nms'] += int(torch.sum(keep_index))
        #print(f'total removal before nms: {torch.sum(keep_index)}')
        ...
        torch.cuda.synchronize()
        exec_time_dict['nms'] = time.time() - start
        ...
        if not output_stat:
            ...
            return pred_box3d_tensor, scores, bbox_number_dict, exec_time_dict
    ...
    @staticmethod
    def visualize(pred_box_tensor, gt_tensor, pcd, show_vis, save_path, dataset=None, target_tensor=None):
        """
