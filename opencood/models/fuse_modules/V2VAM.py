...
class V2V_AttFusion(nn.Module):
    def __init__(self, feature_dim, temperature=1):
        ...
        self.CCNet = CrissCrossAttention(feature_dim, temperature=temperature)
        self.attn_score_H = None
        self.attn_score_W = None
    ...
    def forward(self, x, record_len, trust_score=None):
        ...
        for xx in split_x:#split_x[0] [num_car, C, W, H]
            ...
            # Pairwise attention operation (attacker can suppress everybody easily)
            for i in range(len(xx[:,0,0,0])):
                att_vehicle = self.CCNet(ego_q, xx[i:i+1], xx[i:i+1])
                if trust_score is not None:
                    att_vehicle = att_vehicle * trust_score[i]
                att.append(att_vehicle)
                #if i==0:
                attn_H.append(self.CCNet.attn_H)
                attn_W.append(self.CCNet.attn_W)
        ...
        if len(attn_H) > 1:
            self.attn_score_H = attn_H[1]
            self.attn_score_W = attn_W[1]
...
class CrissCrossAttention(nn.Module):
    ...
    """
    def __init__(self, in_dim, temperature=1):
    ...
    def forward(self, query, key, value):
        ...
        concate = self.softmax(torch.cat([energy_H / self.temperature, energy_W / self.temperature], 3))
        self.scores = concate
