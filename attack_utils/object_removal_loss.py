...
class objectRemoval(nn.Module):
    def __init__(self, args):
        super(objectRemoval, self).__init__()
    ...
    def forward(self, output_dict, target_dict):
        ...
        box_cls_labels = torch.zeros(target_dict['pos_equal_one'].shape, device=cls_preds.device)
        box_cls_labels = box_cls_labels.view(psm.shape[0], -1).contiguous()

        positives = torch.zeros(size=box_cls_labels.shape, device=cls_preds.device)
        negatives = torch.ones(size=box_cls_labels.shape, device=cls_preds.device)
...
class objectRemoval_target(nn.Module):
    def __init__(self, args):
        super(objectRemoval_target, self).__init__()
        self.reg_loss_func = WeightedSmoothL1Loss()
        self.alpha = 0.25
        self.gamma = 2.0
