...
def dict_tensor_to_list(inputs):
    if isinstance(inputs, list):
        return [dict_tensor_to_list(x) for x in inputs]
    elif isinstance(inputs, dict):
        return {k: dict_tensor_to_list(v) for k, v in inputs.items()}
    elif isinstance(inputs, torch.Tensor):
        return torch_tensor_to_numpy(inputs).tolist()
    else:
        return inputs
